...
Coord=Tuple[int,int]
class ModoManual:
	__slots__='board','player_pos','speed','ed','sizes','S','updated','lock'
	def __init__(self,board,player_pos,lock=None):self.board=board;self.player_pos=player_pos;self.speed=2;self.ed=edificios.edificios;self.sizes={k:tuple(map(int,v.tamanio))for(k,v)in self.ed.items()};self.S=int(BOARD_SIZE);self.updated=False;self.lock=lock or Lock()
	@staticmethod
	...
	def _act_build(self,selected_building):
		...
		with self.lock:
			...
			if not coords:print('Fuera de límites');return selected_building
			if any(c in self.board for c in coords):print('Hay un edificio en la casilla');return selected_building
	...
	def _act_demolish(self,selected_building):
		...
		with self.lock:
			...
			if v is None:print('No hay edificio en la casilla');return selected_building
			def _dem_tuple():
		...
		self.updated=True;self._emit_hud('demolished',bn,rx,ry,rw,rh);self._post_city_changed();return selected_building
