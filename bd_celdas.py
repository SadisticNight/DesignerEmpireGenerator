...
import os,capnp,edificios,struct
...
_CELTMP=_CELDST+'.tmp'
_FLUSH_EVERY=1000 # OPTIMIZADO: Escribe todo SOLO cada 1000 pasos.
_CALLS=0
...
# --- NUEVO: Binary Patching (Escritura directa sin overhead) ---
HEADER_SIZE=128; BYTES_POR_CELDA=64 
def parchear_celda_en_disco(x, y, tipo_id, stats_dict=None):
    try:
        offset=HEADER_SIZE+((x*int(BOARD_SIZE)+y)*BYTES_POR_CELDA)
        data=struct.pack('<H', int(tipo_id)) 
        with open(_CELDST, 'r+b') as f:
...
def update_celdas_bin(board,lock_resources=None):
    ...
    # force_flush()  <-- ELIMINADO PARA OPTIMIZACION
    return msg